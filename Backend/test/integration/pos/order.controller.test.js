...
describe('Order Controller Unit Tests', () => {
  ...
  // ==========================================
  describe('createOrder', () => {
    it('should return 400 for Pre-Order items when pickup_date or pickup_time is missing', async () => {
      req.body = {
        order_type: 'Pre-Order',
        items: [{ product_id: '123', quantity: 1 }],
        pickup_date: '2026-07-25',
    ...
    it('should default order_type to Pre-Order and still require pickup_date/pickup_time when order_type is omitted', async () => {
      req.body = {
        items: [{ product_id: '123', quantity: 1 }],
        // no order_type, no pickup_date, no pickup_time
      };
    ...
    it('should NOT require pickup_date/pickup_time for Buy Now orders and should default them', async () => {
      req.body = {
        order_number: 'ORD-200',
        customer_name: 'Michelle De Guzman',
        order_type: 'Buy Now',
        payment_type: 'full',
        items: [{ product_id: 'prod-2', quantity: 1 }],
        // no pickup_date, no pickup_time
      };

      const mockCreatedOrder = { id: 'order-uuid-2', order_number: 'ORD-200' };
      OrderService.createOrderWithItems.mockResolvedValue(mockCreatedOrder);
    ...
    it('should use the client-provided pickup_date/pickup_time for Buy Now when present', async () => {
      req.body = {
        order_type: 'Buy Now',
      ...
      OrderService.createOrderWithItems.mockResolvedValue({ id: 'order-uuid-3' });
    ...
    describe('payment_type normalization', () => {
      const cases = [
        ['full', 'full'],
        ['deposit', 'deposit'],
        ['cash', 'full'],
        ...
        ['partial', 'deposit'],
        [undefined, 'full'],
        ['something-unrecognized', 'full'],
      ];

      it.each(cases)('maps payment_type "%s" to "%s"', async (input, expected) => {
        req.body = {
          order_type: 'Buy Now',
          payment_type: input,
        ...
        OrderService.createOrderWithItems.mockResolvedValue({ id: 'order-uuid' });

        await orderController.createOrder(req, res, next);

        expect(OrderService.createOrderWithItems).toHaveBeenCalledWith(
          expect.objectContaining({ payment_type: expected })
        );
      });
    });
